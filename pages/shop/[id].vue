<template>
  <div class="min-h-screen pt-28 px-5 pb-5 relative">
    ...
    <div v-if="!isLoading" class="product flex lg:flex-row flex-col gap-5">
        <!-- img -->
        <img :src="selectedImage" class="h-[30rem] bg-cover object-cover" alt="">
         <!-- details -->
          <div class="space-y-3 w-full">
            ...
            <div class="space-x-2 pt-2">
                <label @click="selectedSize = size" :class="selectedSize === size ? 'border-2 border-neutral-800' : 'border-neutral-400 border-[1px]' " v-for="size in product.sizes" :key="size.id" class="px-3 py-2 hover:cursor-pointer text-xl">{{ size }}</label>
            </div>
            ...
            <!-- multiple images -->
             <div class="flex items-center gap-2">
                <img @click="selectedImage = img" :class="selectedImage === img ? 'border-2 border-neutral-800' : 'border-neutral-400 border-[1px]'" class="hover:cursor-pointer h-16 w-16 bg-cover object-cover" :src="img" alt="" v-for="img in product.imgUrls" :key="img.id">
             </div>
